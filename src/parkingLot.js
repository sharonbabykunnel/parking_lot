class ParkingLot {
    ...
    status() {
        let output = 'Slot No.\tRegistration No.\TColor\n';
    ...
    getRegistrationNumbersForColor(color) {
        const registrationNumber = this.slots
            .filter(slot => slot !== null && slot.color.toLowerCase() === color.toLowerCase())
            .map(slot => slot.registrationNo);
        return registrationNumber.length > 0 ? registrationNumber.join(' ') : "Not found";
    }

    getSlotNumberForColor(color) {
    ...
    getSlotNumberForRegistration(registrationNo) {
        const slotNumber = this.slots.findIndex(slot => slot !== null && slot.registrationNo === registrationNo);
